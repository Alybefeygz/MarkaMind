...
import { createChatbox, getUserChatboxes, assignPendingPDFsToChatbox, type ChatboxCreate, type ChatboxResponse } from '@/lib/api'
...
export default function Home() {
  ...
  // ✅ FIX: Default chatbox değerleri
  const getDefaultChatboxData = () => ({
    name: '',
    chatbox_title: '',
    initial_message: '',
  ...
  // ✅ FIX: localStorage'tan yükle veya default değerleri kullan
  const [newChatboxData, setNewChatboxData] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('draft_chatbox_data')
      if (saved) {
        try {
          return JSON.parse(saved)
        } catch {
          return getDefaultChatboxData()
        }
      }
    }
    return getDefaultChatboxData()
  ...
  // ✅ FIX: Auto-save draft chatbox data to localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('draft_chatbox_data', JSON.stringify(newChatboxData))
    }
  }, [newChatboxData])
  ...
  const handlePageChange = (pageIndex: number) => {
    ...
    // ✅ FIX: Chatbox oluşturma modundan çıkış - localStorage'ı KORUMAK
    // Kullanıcı tekrar geldiğinde doldurduğu verileri görsün
  ...
  return (
    <div className="min-h-screen bg-[#F9F9FB] font-sans">
      ...
      <div className="flex h-screen">
        ...
        <div className={`transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-16 sm:w-20 md:w-24' : 'w-3 sm:w-4 md:w-6'
          }`}></div>

        <div className={`flex-1 p-3 sm:p-4 md:p-6 lg:p-8 overflow-y-auto overflow-x-hidden transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-0.5 sm:ml-0.5 md:ml-1' : 'ml-0.5 sm:ml-0.5 md:ml-1'
          }`}>
          {/* Header */}
          <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 mb-4 sm:mb-6 relative mt-4 sm:mt-2 md:mt-0">
            <div className="flex items-center flex-1">
              ...
              <button
                ...
                className={`p-1.5 sm:p-2 rounded-lg transition-all duration-300 group mr-2 sm:mr-3 md:mr-4 ${isSidebarOpen ? 'opacity-0 translate-x-6 sm:translate-x-7 md:translate-x-8 pointer-events-none' : 'opacity-100 translate-x-0'
                  }`}
                style={{
            ...
            {pageTitle.bold === 'Chatbox' && pageTitle.normal === 'Yönetimi' && (
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 lg:space-x-4 w-full sm:w-auto">
                ...
                <button
                  onClick={() => {
                    // ✅ FIX: localStorage'tan yükle (varsa), yoksa default kullan
                    const saved = localStorage.getItem('draft_chatbox_data')
                    if (saved) {
                      try {
                        setNewChatboxData(JSON.parse(saved))
                      } catch {
                        setNewChatboxData(getDefaultChatboxData())
                      }
                    } else {
                      setNewChatboxData(getDefaultChatboxData())
                    }
                  ...
                  <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-6 xl:space-x-8 min-w-max">
                    {chatboxTabs.map((tab, index) => (
                      <button
                        ...
                        className={`pb-2 sm:pb-3 font-medium transition-colors duration-200 relative text-xs sm:text-sm lg:text-base whitespace-nowrap px-1 sm:px-2 ${activeTab === tab
                            ? ''
              ...
              <div className="border-b border-gray-200 relative overflow-x-auto scrollbar-hide overflow-y-visible">
                <div className="flex items-center justify-between ml-1 sm:ml-2 lg:ml-6 xl:ml-14 mr-1 sm:mr-2 lg:mr-6 xl:mr-14 px-2 sm:px-0 py-1">
                  ...
                  <div className="flex items-center space-x-2 sm:space-x-3 pb-2">
                    ...
                    <button
                      onClick={() => {
                        // ✅ FIX: localStorage'ı KORUMAK - kullanıcı tekrar geldiğinde verileri görsün
                        // Sadece modu kapat
                        setIsCreatingNewChatbox(false)
                        setActiveTab('Önizleme') // Default tab'a dön
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          // ✅ Pending PDF'leri bu chatbox'a ata
                          try {
                            console.log('📂 Pending PDF\'ler atanıyor...')
                            const assignResult = await assignPendingPDFsToChatbox(result.id)
                            console.log('✅ PDF Atama Sonucu:', assignResult)
                          } catch (assignError) {
                            console.error('⚠️ PDF atama hatası:', assignError)
                            // Ana akışı bozma, sadece logla
                          }
                          ...
                          // ✅ FIX: localStorage'ı temizle ve formu sıfırla
                          localStorage.removeItem('draft_chatbox_data')
                          setNewChatboxData(getDefaultChatboxData())
                          setIsCreatingNewChatbox(false)
                          setPageTitle({ bold: 'Chatbox', normal: 'Yönetimi' })
                        } catch (error) {
